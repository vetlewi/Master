...
public:
    ...
    inline float operator()(const float x) const { return Eval(x); }
private:
...
public:
    ...
    //! Evaluate energy.
    inline float Eeval(const float x) const { return epar(x); }
...
class GainParameters : public Parameters
{
public:
    //! Constructor
    ...
    GainParameters(const std::string &filename) : Parameters(filename){ }
