...
private:
    ...
    //! Finds the words to include in the event.
    /*! Method to fill the event strucure. When invoked it will add all data words with EventTime - 35 \f$\leq\f$ timestamp \f$\leq\f$ EventTime + 150.
     *  \return true if successful, false if not.
     */
    bool ExtractEvent(Event& event,         /*!< The event structure to fill.                   */
                      int& n_data,          /*!< Amount of data extracted.                      */
                      int i,                /*!< First word in the event.                       */
                      int64_t &EventTime    /*!< Variable to place the timestamp of the event.  */);
};
